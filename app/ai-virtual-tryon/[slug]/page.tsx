...
const SlugTryOnPage: React.FC = () => {
  ...
  const handleTryOn = async () => {
    ...
    try {
      ...
      const traceHeader = genResp.headers.get('X-TraceId') || genResp.headers.get('x-traceid') || traceId;
      diag('server-response-meta', { status: genResp.status, contentType, traceId: traceHeader });
      setResultImgLoading(true);
      setIsResultPending(true);
      emitUI('before-set-url', { urlKind: 'server-cos-original' });
      let cosBlob: Blob | null = null;
      try {
        const cosResp = await fetch(`/api/virtual-tryon/result/${traceHeader}?ext=png`, { cache: 'no-store' });
        if (cosResp.ok) {
          cosBlob = await cosResp.blob();
        } else {
          const txt = await cosResp.text().catch(() => '');
          diag('cos-result-fetch-failed', { status: cosResp.status, text: txt.slice(0, 200) });
        }
      } catch (e: any) {
        diag('cos-result-fetch-error', e?.message || String(e));
      }
      if (!cosBlob) {
      ...
      const objUrl = URL.createObjectURL(cosBlob);
      setResultUrl(objUrl);
      setShowResult(true);
      if (progressIntervalRef.current) { try { window.clearInterval(progressIntervalRef.current); } catch {} progressIntervalRef.current = null; }
      setProcessingProgress(100);
      ...
      try {
        const reader = new FileReader();
        const dataUrl: string = await new Promise((resolve, reject) => { reader.onloadend = () => resolve(reader.result as string); reader.onerror = (e) => reject(e); reader.readAsDataURL(cosBlob as Blob); });
      ...
      const baseDataUrl = baseResultRef.current || '';
      if (!baseDataUrl) {
